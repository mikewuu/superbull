...
Sign-in is Google via `@convex-dev/auth`; a personal workspace is created on first sign-in. Live queue actions reach a connector through the gateway (`apps/gateway`); events arrive via Convex ingest.
...
| `RESEND_API_KEY`, `EMAIL_FROM` | Alert and digest email delivery |
