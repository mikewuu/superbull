      ...
      <table className="w-full text-left text-[13px]">
        <thead>
          <tr className="border-b border-border-subtle bg-bg-muted text-content-subtle">
            {headers.map((header) => (
              <th key={header} className="px-4 py-2.5 font-medium whitespace-nowrap first:w-px">
                {header}
              </th>
            ))}
          </tr>
            ...
            <tr key={row.join('|')}>
              {row.map((cell, cellIndex) => (
                <td
                  ...
                  key={cellIndex}
                  className="px-4 py-2.5 align-top font-mono text-[12.5px] whitespace-normal break-words text-content-default first:w-px first:whitespace-nowrap first:text-content-emphasis"
                >
                  {cell}
                </td>
              ))}
