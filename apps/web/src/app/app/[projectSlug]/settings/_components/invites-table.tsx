...
export function InvitesTable(props: InvitesTableProps) {
  const { projectSlug, invites } = props;
          ...
          {pending.map((invite) => (
            ...
            >
              ...
              <td className="px-5 py-3 text-right">
                <RevokeInviteButton projectSlug={projectSlug} inviteId={invite._id} />
              </td>
            </tr>
          ))}
        </tbody>
...
function RevokeInviteButton(props: { projectSlug: string; inviteId: Id<'invites'> }) {
  const { projectSlug, inviteId } = props;
  const [pending, startTransition] = useTransition();

  const handleRevoke = () => {
    startTransition(async () => {
      await revokeInviteAction(projectSlug, inviteId);
    });
  };
