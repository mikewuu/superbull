...
export function ErrorGroupActions(props: ErrorGroupActionsProps) {
  const { projectSlug, groupId, state } = props;
  const [confirmingIgnore, setConfirmingIgnore] = useState(false);
  const [pending, startTransition] = useTransition();

  const handleResolve = () => {
    startTransition(async () => {
      await resolveErrorGroupAction(projectSlug, groupId);
    });
  };

  const handleReopen = () => {
    startTransition(async () => {
      await reopenErrorGroupAction(projectSlug, groupId);
    });
  };

  const handleConfirmIgnore = () => {
    startTransition(async () => {
      await ignoreErrorGroupAction(projectSlug, groupId);
