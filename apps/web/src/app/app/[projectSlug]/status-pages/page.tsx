...
export default async function StatusPagesPage(props: StatusPagesPageProps) {
  ...
  const rows = await getStatusPageRows(project._id, connectors);
                ...
                {rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-right">
                      <a
                        href={`/app/${projectSlug}/status-pages/${row.connector.id}`}
...
async function getStatusPageRows(
  ...
  const configs = await Promise.all(
    connectors.map((connector) =>
      getStatusPageConfig({ projectId, connectorId: connector.id as Id<'connectors'> }),
    ),
  );
