...
import { type CreateAlertRuleActionState, createAlertRuleAction } from '../actions';

const initialState: CreateAlertRuleActionState = { error: null };
...
export function NewRuleDialog(props: NewRuleDialogProps) {
  const { workspaceSlug, connectors } = props;
  const [showing, setShowing] = useState(false);
  const [type, setType] = useState<AlertRuleType>('failed_threshold');
  const [state, formAction, pending] = useActionState(
    createAlertRuleAction.bind(null, workspaceSlug),
    initialState,
  );
    ...
    <>
      ...
      <Dialog showing={showing} onClose={() => setShowing(false)} title="New alert rule">
        <form action={formAction} className="space-y-3">
          ...
          <div>
            <label htmlFor="connectorId" className="block text-xs font-medium text-content-subtle">
              Connector
            </label>
            <select
              id="connectorId"
