...
import { listAnnotationsByWorkspace } from '../../../../../lib/deploy-annotations/list-annotations-by-workspace';
...
export default async function ErrorGroupDetailPage(props: ErrorGroupDetailPageProps) {
  const { workspaceSlug, groupId } = await props.params;
  const { workspace } = await requireWorkspaceForSlug(workspaceSlug);
  const group = await findErrorGroupById(workspace._id, groupId as Id<'errorGroups'>);
  if (!group) {
    notFound();
  }
  ...
  return (
    <>
      <PageHeader
        title={
          <Breadcrumbs
            items={[
              {
                label: 'Errors',
                to: `/app/${workspaceSlug}/errors?connector=${group.connectorId}`,
              },
        ...
        controls={
          <ErrorGroupActions workspaceSlug={workspaceSlug} groupId={group.id} state={group.state} />
        }
      />
...
async function getDeployContext(
  ...
  const annotations = await listAnnotationsByWorkspace(workspaceId, connectorId);
