...
export function ErrorGroupActions(props: ErrorGroupActionsProps) {
  const { workspaceSlug, groupId, state } = props;
  const [confirmingIgnore, setConfirmingIgnore] = useState(false);
  const [pending, startTransition] = useTransition();

  const handleResolve = () => {
    startTransition(async () => {
      await resolveErrorGroupAction(workspaceSlug, groupId);
    });
  };

  const handleReopen = () => {
    startTransition(async () => {
      await reopenErrorGroupAction(workspaceSlug, groupId);
    });
  };

  const handleConfirmIgnore = () => {
    startTransition(async () => {
      await ignoreErrorGroupAction(workspaceSlug, groupId);
