...
export function LogoUpload(props: LogoUploadProps) {
  const { workspaceSlug, configId, connectorId, hasLogo } = props;
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadUrl = await generateLogoUploadUrlAction(workspaceSlug);
      ...
      await setStatusPageLogoAction(workspaceSlug, { configId, connectorId, storageId });
    } finally {
      setUploading(false);
    }
  };
