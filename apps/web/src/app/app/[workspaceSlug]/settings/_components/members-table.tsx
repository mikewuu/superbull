...
export function MembersTable(props: MembersTableProps) {
  const { workspaceSlug, members, canManage, currentUserId } = props;
            ...
            >
              ...
              <td className="px-5 py-3 text-right">
                {canManage && member.role !== 'owner' && (
                  <RemoveMemberButton workspaceSlug={workspaceSlug} memberId={member._id} />
                )}
              </td>
            </tr>
          ))}
...
function RemoveMemberButton(props: { workspaceSlug: string; memberId: Id<'members'> }) {
  ...
  const handleRemove = () => {
    startTransition(async () => {
      const result = await removeMemberAction(workspaceSlug, memberId);
