    ...
    <Html>
      ...
      <Body style={bodyStyle}>
        <Container style={containerStyle}>
          ...
          {perConnector.length === 0 && <Text style={metaStyle}>No connectors reporting yet.</Text>}
          {perConnector.map((connector) => (
            <Section key={connector.connectorId} style={sectionStyle}>
              <Text style={sourceNameStyle}>{connector.connectorName}</Text>
              <Text style={metaStyle}>
                {connector.completed} completed &middot; {connector.failed} failed (24h)
              </Text>
              {connector.topErrorGroups.length > 0 && (
                <>
                  <Text style={subheadStyle}>Top errors</Text>
                  {connector.topErrorGroups.map((group) => (
