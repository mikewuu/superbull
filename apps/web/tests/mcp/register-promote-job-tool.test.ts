...
describe('registerPromoteJobTool', () => {
  it('registers a destructive tool named promote_job', () => {
    const { server, tools } = createFakeServer();
    registerPromoteJobTool(server);

    expect(tools.get('promote_job')?.config.annotations).toMatchObject({ destructiveHint: true });
  });

  it('PUTs the promote route and reports success on 204', async () => {
