...
Private workspace app (not published). The always-on WebSocket service at `connect.superbull.com` that every [@superbull/connector](https://www.npmjs.com/package/@superbull/connector) opens its single outbound connection to. It authenticates connectors, relays RPC from the hosted web app to the right connector, and forwards connector events into Convex.
...
- `POST /internal/rpc`: bearer-authed (gateway internal token) RPC bridge the web app calls; forwards a `request` frame to the connector and awaits its `response` (10s timeout; 502 when disconnected, 504 on timeout).
